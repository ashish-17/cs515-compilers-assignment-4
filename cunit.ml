...
| GLabelOffset of Rux86.lbl * int32
| GLabels of Rux86.lbl list
| GInt32 of int32
| GZero  of int          
| GExtern                
...
let string_of_global_data (d:global_data) =
  let data_defn =
    ...
    | GLabelOffset (l, i) ->
        "\t.long " ^ (Rux86.string_of_lbl l) ^ " + " ^ (Int32.to_string i)
    | GLabels ls -> List.fold_left 
        (fun s -> fun l -> s ^ "\t.long " ^ Rux86.string_of_lbl l ^ "\n") "" ls
    | GInt32 v -> "\t.long " ^ Int32.to_string v
    | GZero z -> "\t.zero " ^ string_of_int z
    | GExtern -> ""  in
  let maybe_global =
    if d.link || d.value = GExtern then 
      ".globl " ^ Rux86.string_of_lbl d.label ^ "\n"
    else "" in
    maybe_global ^ 
      (if (d.value <> GExtern) then
         Rux86.string_of_lbl d.label ^ ":\n" ^ data_defn ^ "\n"
       else "")
    ...
    (List.fold_left 
	 ...
	 let mode' =
	   ...
	   | Code c ->
	       ...
	       Rux86.serialize_insn_block c printfn;
	       mode_text
	   | Data d ->
